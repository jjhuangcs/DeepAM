...
%   previously 
function [f,q_weight] = Sparsifying_functions_norm_joint(Type, Purpose, Omega, R, X, Exponents, mu, n, q_weight)
...
switch Type
    ...
    case 'LogSquare'
        % log(1+mu*x^2)
        if ~derivative || Comp_qweight
            q_weight = sum(reshape(log(1+mu*((Omega*X).^2 - (Omega*R).^2).^2),n,[]))/log(1+mu);
        end
        if derivative || Comp_qweight
            f = mu*2*((Omega*X).^2 - (Omega*R).^2)./(1+mu*((Omega*X).^2 - (Omega*R).^2).^2)/log(1+mu);
        end
    case 'LogRatio'
        if ~derivative || Comp_qweight
            OX = Omega*X;
            OR = Omega*R;
            q_weight = sum(reshape(log((OX).^2./((OR).^2+1e-6)+(OR).^2./((OX).^2+1e-6)),n,[]));
        end
        if derivative || Comp_qweight
            f1 = (1./((Omega*X).^4+(Omega*R).^4+1e-6));
            f = 4*f1.*(Omega*X).^3*X' +4*f1.*(Omega*R).^3*R';
            f = f -2*(1./((Omega*X).^2+1e-6)).*(Omega*X)*X';
            f = f -2*(1./((Omega*R).^2+1e-6)).*(Omega*R)*R';
        end
    case 'LogAbsSquare'
        % log(1+mu*(abs(x)-abs(r))^2)
        if ~derivative || Comp_qweight
            q_weight = sum(reshape(log(1+mu*(abs(Omega*X) - abs(Omega*R)).^2),n,[]))/log(1+mu);
        end
        if derivative || Comp_qweight
            f = mu*2*(abs(Omega*X) - abs(Omega*R))./(1+mu*(abs(Omega*X) - abs(Omega*R)).^2)/log(1+mu);
        end
...
if derivative
    f = reshape(bsxfun(@times,reshape(f,n,[]),q_weight.^(q-1)),size(Omega*X));
end
